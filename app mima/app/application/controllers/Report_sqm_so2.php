...
class Report_sqm_so2 extends MY_Controller
{
    ...
    function index()
    {
        ...
        if($id_client_group==6){
            $view_data["iframe_src"] = "http://localhost:3000/react/so2";
            $this->template->rander("report_sqm_pm10/index", $view_data);
        } else if($id_client_group==7){
            $view_data["iframe_src"] = "http://localhost:3000/react/so2-grup1";
            $this->template->rander("report_sqm_pm10/index", $view_data);
        } else {
            $view_data["iframe_src"] = "http://localhost:3000/react/so2-grup2";
            $this->template->rander("report_sqm_pm10/index", $view_data);
        }
    }
}
